...
public class CustomerRepository {
	...
	public List<CustomerDTO> findCustomers(int page, int size) {
		...
		return jdbcTemplate.query(sql, ps -> {
			ps.setInt(1, size);
			ps.setInt(2, offset);
		}, (rs, rowNum) -> CustomerDTO.builder().customerId(rs.getString("C_CUSTOMER_ID"))
				.firstName(rs.getString("C_FIRST_NAME")).lastName(rs.getString("C_LAST_NAME"))
				.email(rs.getString("C_EMAIL_ADDRESS")).birthDay(rs.getInt("C_BIRTH_DAY"))
				.birthMonth(rs.getInt("C_BIRTH_MONTH")).birthYear(rs.getInt("C_BIRTH_YEAR"))
				.address(AddressDTO.builder().street(rs.getString("CA_STREET_NAME"))
						.number(rs.getString("CA_STREET_NUMBER")).city(rs.getString("CA_CITY"))
						.state(rs.getString("CA_STATE")).country(rs.getString("CA_COUNTRY")).build())
				.build());
	}
